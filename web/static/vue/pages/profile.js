const profileApp = Soumetsu.createApp({
  data() {
    return {
      ...
      discordUser: null,
      commentsInfo: { disabled: false, total: 0 },
  ...
  methods: {
    ...
    async loadUserData() {
      ...
      try {
        ...
        const userResp = await this.api(`users/${userId}`, {
          mode: this.mode,
    ...
    renderChart() {
      ...
      if (!chartEl || !this.graphData?.length) {
        return;
      }
    ...
    async loadScores(type) {
      ...
      if (scoreData.loading || !scoreData.hasMore) {
        return;
      }
    ...
    async loadComments() {
      ...
      this.commentsInfo.total = this.comments.length;
    },

    async postComment() {
      if (!this.commentText.trim() || this.commentText.length > 380 || this.commentPosting) {
        return;
      }
      ...
      try {
        await SoumetsuAPI.comments.create(this.userID, this.commentText.trim());
        this.commentText = '';
        this.comments = [];
        this.commentPage = 0;
    ...
    async deleteComment(id) {
      ...
      try {
        await SoumetsuAPI.comments.delete(id);
    ...
    // Friend actions
    async toggleFriend() {
      if (this.friendLoading || !this.canInteract) {
        return;
      }
      ...
      try {
        if (isAdding) {
          await SoumetsuAPI.friends.add(this.userID);
          this.friendStatus = 1;
          this.followers.follower_count++;
        } else {
          await SoumetsuAPI.friends.remove(this.userID);
          this.friendStatus = 0;
          this.followers.follower_count--;
        }
      } catch (err) {
    ...
    async loadUserStats() {
      try {
        const userResp = await this.api(`users/${this.userID}`, {
          mode: this.mode,
          custom_mode: this.customMode,
        });
        if (userResp?.stats) {
    ...
    async togglePin() {
      ...
      try {
        if (isPinned) {
          await SoumetsuAPI.scores.unpin(this.pinModalScore.id);
        } else {
          await SoumetsuAPI.scores.pin(this.pinModalScore.id);
        }
    ...
    getBadgeIconClass(icon) {
      ...
      const colorFaIconMatch = strippedIcon.match(
        new RegExp('^(' + colorPattern + ')\\s*fa-(.+)$', 'i')
      );
      if (colorFaIconMatch) {
      ...
      const colorIconMatch = strippedIcon.match(
        new RegExp('^(' + colorPattern + ')\\s*(.+)$', 'i')
      );
      if (colorIconMatch) {
