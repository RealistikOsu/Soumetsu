  ...
  methods: {
    async loadClanData() {
      ...
      try {
        ...
        await Promise.all([this.loadMembers(), this.loadModeData()]);
    ...
    async loadModeData() {
      await Promise.all([this.loadStats(), this.loadTopPlays(), this.loadMemberLeaderboard()]);
    },
    ...
    async loadTopPlays() {
      ...
      try {
        const data = await SoumetsuAPI.clans.getTopScores(
          this.clanId,
          this.mode,
          this.customMode,
          4
    ...
    async loadMemberLeaderboard() {
      ...
      try {
        const data = await SoumetsuAPI.clans.getMemberLeaderboard(
          this.clanId,
          this.mode,
          this.customMode
        );
