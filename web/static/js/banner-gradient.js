...
(function () {
  ...
  function extractBannerColours(img, callback) {
    ...
    try {
      ...
      colourCandidates.sort(function (a, b) {
  ...
  function applyBannerGradient(bannerElement, colours) {
    ...
    if (colours && colours.colour1 && colours.colour2) {
      ...
      bannerElement.style.background =
        'linear-gradient(to bottom right, ' + colour1RGBA + ', ' + colour2RGBA + ')';
    } else {
      bannerElement.style.background = '';
    }
  }
  ...
  function initBannerGradient(avatarSelector, bannerSelector, options) {
    options = options || {};
    const avatar =
      typeof avatarSelector === 'string' ? document.querySelector(avatarSelector) : avatarSelector;
    const banner =
      typeof bannerSelector === 'string' ? document.querySelector(bannerSelector) : bannerSelector;
